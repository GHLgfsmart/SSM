...
function add(){
	...
	for(var i=0;i < document.getElementsByName('matid').length;i++){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=basePath%>warehousing/output_storageAddPage.do?MATID='+str;
	 diag.Width = 850;
	 diag.Height = 500;
...
function examine(Id,state){
	if(state!=0){
		...
		return;
		 ...
		 diag.Drag=true;
		 diag.Title ="检验";
		 diag.URL = '<%=basePath%>warehousing/outputcheckoutAddPage.do?yan=rk&ID='+Id;
		 diag.Width = 850;
		 diag.Height = 500;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
function del(Id){
	bootbox.confirm("确定要删除吗?", function(result) {
		if(result) {
			top.jzts();
			var url = '<%=basePath%>warehousing/output_storageDel.do?ID='+Id;
...
function makeAll(msg){
	...
	if(str==''){
		...
		bootbox.confirm(msg, function(result) {
			if(result) {
				top.jzts();
				var url = '<%=basePath%>warehousing/output_storageBatchDel.do?DATA_IDS='+str;
				$.get(url,function(data){
					$(top.hangge());//关闭加载状态
					if(data == 'success'){
						swal({   

			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
					<form action="car/list.do" method="post" name="carForm" id="carForm">
									...
									<c:forEach items="${carList}" var="car" varStatus="vs">
										...
										<tr id="t${vs.index}">
											<td class='center' style="width: 30px;">
												<c:if test="${user.USERNAME != 'admin'}"><label><input type='checkbox' name='ids' value="${car.ID }" class="ace"/><span class="lbl"></span></label></c:if>
											...
											<td class="center">赣B ${car.BRAND }</td>
											<td class="center"><i class="ace-icon fa fa-clock-o bigger-110 hidden-480"></i>${car.TIMES }</td>
											<td class="center">${car.WEIGHT }<B>/t</B></td>
											<td class="center">
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
														<a class="btn btn-xs btn-success" title="编辑" onclick="editUser('${car.ID}');">
															<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
														</a>
													</c:if>
													<c:if test="${QX.del == 1 }">
														<a class="btn btn-xs btn-danger" onclick="delDriver('${car.ID }');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
														  <c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="editUser('${car.ID}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
														  ...
														  <c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="delUser('${car.ID }');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
...
function delDriver(ID){
	bootbox.confirm("确定要删除该信息吗?", function(result) {
		if(result) {
			top.jzts();
			var url = "<%=basePath%>car/deleteU.do?ID="+ID;
		...
		if(result) {
			...
			if(str==''){
				...
				if(msg == '确定要删除选中的数据吗?'){
					...
					$.ajax({
						type: "POST",
						url: '<%=basePath%>car/deleteAllU.do?tm='+new Date().getTime(),
				    	data: {IDS:str},
						dataType:'json',
						cache: false,
...
function toExcel(){
	...
	window.location.href='<%=basePath%>car/excel.do?keywords='+keywords;
}

			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="goods/testPage3.do" method="post" name="Form" id="Form">
								...
								<c:when test="${not empty varList}">
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr id="t${vs.index}">
											...
											<td class='center'><label><input type='hidden' name='count' value="${var.COUNT}" class="ace" /></label>${var.COUNT}</td>
											<td class='center'><label><input type='hidden' name='matcount' value="${var.MATCOUNT}" class="ace" /></label>${var.MATCOUNT}</td>
...
function idsid() {
	var idsid2=window.parent.window.idsid2;
	var strs= new Array(); //定义一数组 
	strs=idsid2.split(";"); //字符分割
...
function elect(msg){
	...
	for(var i=0;i < document.getElementsByName('ids').length;i++){
	  if(document.getElementsByName('ids')[i].checked){
	  	...
	  	if(xcount=='') xcount += document.getElementsByName('count')[i].value;
	  	if(xmatcount=='') xmatcount += document.getElementsByName('matcount')[i].value;
	 	c=c+1;
	  }
	}
	if(xid==''){
		...
		window.parent.window.m_count = xcount;
		window.parent.window.MATCOUNT = xmatcount;
		window.parent.window.mid = xid;
		window.parent.window.idsid2 = idsid;

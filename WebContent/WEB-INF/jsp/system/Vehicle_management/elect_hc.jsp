			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="goods/testPage2.do" method="post" name="Form" id="Form">
								...
								<c:when test="${not empty varList}">
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr id="t${vs.index}">
											...
											<td class='center'><label><input type='hidden' name='brand' value="${var.BRAND}" class="ace" /></label>${var.BRAND}</td>
											<td class='center'>${var.TIMES}</td>
											<td class='center'><label><input type='hidden' name='weight' value="${var.WEIGHT}" class="ace" /></label>${var.WEIGHT }<B>/t</B></td>
										</tr>
...
function idsid() {
	var idsid3=window.parent.window.idsid3;
...
function elect(msg){
	...
	for(var i=0;i < document.getElementsByName('ids').length;i++){
	  if(document.getElementsByName('ids')[i].checked){
	  	...
	  	if(xbrand=='') xbrand += document.getElementsByName('brand')[i].value;
	  	if(xweight=='') xweight += document.getElementsByName('weight')[i].value;
	 	c=c+1;
	  }
	}
	if(xid==''){
		...
		window.parent.window.m_brand = xbrand;
		window.parent.window.m_weight = xweight;
		window.parent.window.mid = xid;
		window.parent.window.idsid3 = idsid;
